...
마지막 정리일: 2026-07-11 KST (v0.5.0.0 작업 반영)  
...
2026-07-11 v0.5.0.0 작업 요약 (한자 성명학):

- 한자 DB 파이프라인 `scripts/build-hanja-db.mjs`: delvier/krcourt의 webhanja.db(대법원 인명용 한자 SQLite, 약 27MB) → `src/data/name-hanja.json`(음절 490개, 항목 10,381개, 444KB). 원획 계산은 강희 214부수 본자 획수 구간표 + rad_stroke.stroke 합산, 숫자 한자(一~十)만 수의 획으로 덮어씀. 저작권: 법령·국가 편집물이라 저작권법 제7조로 보호 대상 아님(스크립트 주석에 근거 명시)
- 사격 계산 `src/name-hanja.js`: 원형이정(원격=이름 획 합 / 형격=성+첫 자 / 이격=성+끝 자 / 정격=전체 합), 81수리 길흉(길/평/흉) + 키워드, 외자·복성 대응, normalize81(80 초과 시 -80 순환)
- UI: 성명학 섹션에 "한자 이름으로 정밀 사격 풀이" 버튼 → 클릭 시 name-hanja.json을 동적 import(별도 청크). 음절별 인명용 한자 후보 셀렉트(뜻·원획 표기), 복성 체크박스(3음절+), 4격 결과 카드(길=옥색/평=금색/흉=빨강), 유파 차이 고지
- 검증: 원획 스팟체크(智12·洙10[氵→水 보정]·五5[수의]·金8), 사격 예제(金智洙 → 원격22흉·형격20흉·이격18길·정격30평), 81수리 전수 분류, 외자 이격=형격 동일. 테스트 52개 통과, 빌드 성공, **브라우저 검증 완료**(김지수→金智洙 실제 렌더·콘솔 오류 없음 스크린샷 확인)
- 이벤트: hanja-open / hanja-reading GoatCounter 계측 추가

2026-07-10 v0.4.1.0 작업 요약:

- 공유 딥링크 `src/share.js`: `#r=s|l|ll.YYYY-MM-DD.HH.MM[.topic]` 인코딩/디코딩. 이름·고민·톤 절대 미포함, 형식 위반 8종 테스트로 차단, 존재하지 않는 날짜는 calculateChart에서 차단 후 오류 안내
- 부트스트랩 분기 순서 확정: 해시 > 저장 프로필 > 빈 폼. 공유 뷰는 일회성 렌더(프로필 비파괴), "내 사주로 돌아가기", hashchange 처리, 렌더 후 replaceState로 해시 제거
- 결과 링크 공유 버튼: Web Share → 클립보드 폴백, 개인정보 고지 문구
- GoatCounter 활성화(사이트 코드 fortune9388) + share-link/share-open 이벤트 추가
- 테스트 41개 통과, 빌드 성공. 공유 플로우 브라우저 상호작용 검증은 프리뷰 도구 재연결 후 필요(로직은 단위 테스트로 커버)
