...
- Gemini-backed consultation message handling with the `send-consultation-message` Edge Function
...
npx --yes supabase functions deploy send-consultation-message --use-api
```
...
- Set `GEMINI_API_KEY` and `GEMINI_MODEL` only in Supabase Edge Function secrets or another server runtime.
