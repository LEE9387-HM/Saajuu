...
## 14. 추가 수익화 방향 후보 (2026-07-08)

1~14장의 본선(리포트 → 상담 → 월간권)과 별개로, 같은 자산(사주 엔진 + 해석 콘텐츠 + 트래픽)에서 추가 매출을 뽑는 후보들. 본선을 흔들지 않는 선에서 실험 비용이 낮은 것부터 붙인다.

### 14.1 지금 정적 페이지 상태로도 가능한 것 (서버 불필요)

| 후보 | 내용 | 매력 | 주의 |
|------|------|------|------|
| **디지털 굿즈** | 결과 화면에서 "나의 사주 카드" 고화질 이미지·오행 월페이퍼·사주 한 장 PDF를 1,900~2,900원에 판매 | 원가 0, 충동구매 가격대, 이미 계산된 데이터의 재포장 | 결제 수단 필요 전까지는 무료 배포로 공유(바이럴) 용도부터 |
| **복채 모델** | 무료 풀이 끝에 "마음에 들었다면 복채를 남겨주세요" 자율 후원 | 사주 문화와 정합. 결제 인프라 전 수요 검증 수단 | 개인 간 송금 링크의 PG·세금 규정 확인 후 도입 |
| **콘텐츠 채널 유입** | "오늘의 일진/띠별 운세"를 쇼츠·스레드·인스타로 자동 생성해 채널 성장 → 채널 수익 + 앱 유입 | 엔진이 이미 있어 콘텐츠 원가가 0에 수렴 | 채널 운영 시간이 든다. 자동화 파이프라인 먼저 |
| **문맥 제휴(CPA)** | 주제별 결과에 맞는 제휴: 결혼운→웨딩 플랫폼, 사업운→사업자 서비스, 신년→다이어리/굿즈 | 디스플레이 광고 대비 단가 수배 | 금융 상품(대출·보험) 리드젠은 규제 확인 전 금지 |

### 14.2 서버 도입 후 (Phase 1~2에 편승)

| 후보 | 내용 | 매력 | 주의 |
|------|------|------|------|
| **선물하기** | 궁합·신년 리포트를 친구에게 선물하는 결제 링크 (생일·연말 시즌) | 결제와 신규 유입이 한 번에 발생. 운세는 선물 적합 카테고리 | 받는 쪽 UX(생년월일 입력)를 선물 코드 방식으로 |
| **택일 리포트** | 결혼식·이사·개업 날짜 후보 3개 + 근거. 19,000~49,000원 | 의도가 극도로 명확한 고단가 수요. "날짜"라는 구체적 산출물이라 만족도 측정 쉬움 | 단정 표현 금지 원칙과 충돌하지 않게 "추천 후보 + 근거" 형식 유지 |
| **주간 운세 뉴스레터** | 무료 이메일 뉴스레터 → 이메일 자산 축적 → 시즌 상품 판매 채널 | 신년운세 성수기(Phase 4)의 판매 채널을 미리 만드는 효과 | 개인정보(이메일+생년월일) 처리방침 필요 |

### 14.3 장기 (본선 검증 후)

| 후보 | 내용 | 매력 | 주의 |
|------|------|------|------|
| **B2B API/위젯** | 만세력+해석 JSON API, 임베더블 궁합 위젯을 데이팅·웨딩·콘텐츠 업체에 판매 | 점신도 B2B 콘텐츠 공급이 수익원. 트래픽 없이도 매출 가능 | 해석 콘텐츠가 충분히 쌓인 뒤에만 차별화됨 |
| **작명·개명 보조 리포트** | 신생아 작명 후보 평가(한자 획수 DB 필요). 오프라인 작명 10~30만 원 시장의 2~5만 원 wedge | 이미 성명학 맛보기 자산 보유. 기존 "성명학 정확도 개선" 과제와 합류 | 한자 DB 확보가 선행 조건 |
| **기업·행사 패키지** | 워크숍/연말 행사용 "팀 궁합" 웹 이벤트, 웨딩페어 부스 패키지 | 건당 수십만 원 B2B 단가 | 운영 공수가 커서 1인 부업과는 상성 확인 필요 |

### 14.4 우선순위 판단

- **가장 먼저**: 디지털 굿즈(무료 공유 카드부터)와 콘텐츠 채널. 본선 Phase 0의 트래픽 확보 과제와 정확히 겹치므로 별도 투자가 아니라 같은 작업의 부산물이다.
- **Phase 1 결제가 열리면**: 선물하기와 택일 리포트를 상품 목록에 추가. 특히 택일은 궁합/결혼 구매자의 자연스러운 다음 구매다.
- **성수기 대비**: 뉴스레터는 11월 신년운세 판매 채널이므로 늦어도 9~10월에 시작.
- **하지 않을 것**: 금융 리드젠(규제), 이용자 데이터 판매(신뢰 파괴), 본선 검증 전 B2B 영업(시간 분산).
