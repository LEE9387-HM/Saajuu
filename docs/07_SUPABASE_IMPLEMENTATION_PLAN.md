...
| `send-consultation-message` | 턴 수 차감, 안전 분류, Gemini 호출, 메시지 저장 |
...
- `GEMINI_API_KEY`: 실제 AI 상담 Edge Function에서 Gemini를 호출할 때 필요하다.
- `GEMINI_MODEL`: 기본 모델명. Free Tier 테스트는 `gemini-2.5-flash` 같은 공개 모델로 시작한다.
- `LLM_PROVIDER`: 현재는 `gemini`로 고정해도 된다. 나중에 OpenAI fallback을 둘 때만 바꾼다.
- `OPENAI_API_KEY`: 향후 fallback 또는 비교 테스트용이다. 지금 단계에서는 비워도 된다.
...
- `send-consultation-message`에서 턴 차감, 안전 분류, Gemini 호출, 메시지 저장을 구현한다.
