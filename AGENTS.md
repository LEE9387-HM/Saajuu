...
2. 반복 개선 루프로 작업할 때는 [LOOP.md](./LOOP.md)를 따르고 결과를 [LOOP_LOG.md](./LOOP_LOG.md)에 남긴다.
3. 작업 방향이 제품/수익화 관련이면 아래 문서를 순서대로 본다.
...
6. `점신/`, `local-reference/`, `data/nemotron-korea/`, `.claude/`는 로컬 참고 자료다.
   공개 저장소에 커밋하지 않는다.
7. 변경 전 `npm test`로 기존 52개 테스트가 통과하는 상태인지 확인하고, 변경 후에도
   `npm test && npm run build`로 검증한다.
8. 작업이 끝나면 `CHANGELOG.md`, `VERSION`(및 `package.json`의 `version`),
   `PROJECT_STATUS_README.md`의 "최근 구현 내용"을 갱신한다.
9. `main`은 푸시 즉시 GitHub Pages에 자동 배포된다(`.github/workflows/deploy-pages.yml`).
   푸시 전에 사용자 승인을 받는다.
